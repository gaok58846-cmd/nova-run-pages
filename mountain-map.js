...
function renderBackground(ctx,state,width,height){
  const floor=floorFor(height),skyKey=`${width}|${height}`;if(state._skyKey!==skyKey){state._skyKey=skyKey;state._skyGradient=ctx.createLinearGradient(0,0,0,height);state._skyGradient.addColorStop(0,'#e8d2ae');state._skyGradient.addColorStop(.58,'#f5dfbd');state._skyGradient.addColorStop(1,'#cbbda5')}ctx.fillStyle=state._skyGradient;ctx.fillRect(0,0,width,height);
...
function render(ctx,state,width,height,theme={}){
  ...
  for(const item of COLLECTIBLES){if(state.collected.has(item.id)||(item.hidden&&!state.hiddenFound))continue;const x=item.x,y=floor+item.y,pulse=state.reducedMotion?1:1+Math.sin(state.time*4+item.x)*.12;ctx.save();ctx.translate(x,y);ctx.scale(pulse,pulse);ctx.shadowBlur=state.reducedMotion||state.renderQuality<.7?0:6;ctx.shadowColor='#f09a31';polygon(ctx,[[0,-14],[11,0],[0,14],[-11,0]],'#f3aa42','#fff4d6');ctx.restore()}
