(()=>{
  ...
  let W=900,H=650,DPR=1,last=0,shake=0,flash=0,visible=true,scheduled=false,frameId=0,sizedW=0,sizedH=0,visualQuality=1,frameTimeEma=16.7,qualityPressure=0,qualityRecovery=0,resizeQueued=false,appUi=null,platform=null,modalResume=false,musicIntensity='normal',cachedHudTop=86;
  const activeControlPointers=new Map();
  const OVERDRIVE_DURATION=4.8,OVERDRIVE_READY_WINDOW=3.2,SIGNATURE_DURATION=5.2;
  ...
  visualQuality=performanceMode==='performance'?.62:1;root.dataset.renderQuality=visualQuality===1?'high':'low';
  ...
  const game={mode:'menu',time:0,score:0,best:(CHALLENGE_SCENES||[]).includes(scene)?Store.challengeRecord(scene).bestScore:loadBest(),distance:0,speed:DIFF[difficulty].startSpeed,combo:1,comboT:0,energy:40,over:0,overReady:false,overWindow:0,overBreaks:0,overPulse:0,overFinish:0,signature:0,signaturePulse:0,tutorialGate:false,perfect:0,flow:1,flowPerfect:0,hitStop:0,spawn:.7,diff:difficulty,lastHazard:'',pendingPlan:null,generatorPhase:'intro',generatorCount:0,checkpointDistance:0,rescueAvailable:false,things:[],particles:[],notes:[],stars:[],towers:[]};
  ...
  const p={x:150,y:0,w:42,h:68,vy:0,jumps:0,ground:true,coyote:PHYSICS.coyoteTime,jumpBuffer:0,jumpHeld:false,slide:0,slideState:'idle',dash:0,shield:0,magnet:0,slow:0,rot:0,land:0,takeoff:0,impact:0,hurt:0,iceDrift:0};
  ...
  const fxBlur=value=>reducedMotion||performanceMode==='performance'?0:value*(root.dataset.theme==='light'?.46:.72);
  ...
  function refreshMusicIntensity(){const next=game.over>0?'overload':game.overReady?'ready':game.signature>0?'highlight':'normal';if(next===musicIntensity)return;musicIntensity=next;if(Sound.setIntensity)Sound.setIntensity(next)}
  function setTutorialGate(value){game.tutorialGate=!!value;root.dataset.tutorialGate=game.tutorialGate?'on':'off';if(game.tutorialGate){stopLoop();Sound.setPlaying(false);render()}else if(game.mode==='play'){Sound.setPlaying(true);last=0;schedule()}}
  function releaseTutorialGate(){if(!game.tutorialGate)return false;setTutorialGate(false);return true}
  function syncModeUi(){const settings=Store.settings(),light=settings.theme==='light';root.dataset.mode=game.mode;root.dataset.scene=scene;root.dataset.theme=light?'light':'dark';root.dataset.touch=touchDevice()?'true':'false';root.style.colorScheme=light?'light':'dark';const themeMeta=document.querySelector('meta[name="theme-color"]');if(themeMeta)themeMeta.content=light?'#f4efe7':'#181919';root.dataset.motion=reducedMotion?'reduced':'full';root.dataset.contrast=settings.highContrast?'high':'normal';root.dataset.performance=performanceMode;root.dataset.overdrive=game.over>0?'on':game.overReady?'ready':'off';root.dataset.signature=game.signature>0?scene:'off';root.dataset.tutorialGate=game.tutorialGate?'on':'off';root.style.setProperty('--touch-opacity',String(settings.touchOpacity));syncQuickUi();Sound.setPlaying(game.mode==='play'&&!game.tutorialGate);refreshMusicIntensity();platform?.setGameMode(game.mode);appUi?.syncPause(game.mode);raf(updateHudTop)}
  ...
  function clearInputState(){Object.assign(mountainInput,{left:false,right:false,down:false,jumpHeld:false,jumpPressed:false,dashPressed:false,autoRun:false});Object.assign(p,{jumpHeld:false,jumpBuffer:0,slide:0,slideState:'idle',dash:0});activeControlPointers.clear();for(const button of root.querySelectorAll('[data-act].is-pressed'))button.classList.remove('is-pressed')}
  function disposeActiveSession(){stopLoop();clearInputState();clearTimeout(orientationHintTimer);orientationHintTimer=0;orientationHint.hidden=true;musicIntensity='normal';if(Sound.setIntensity)Sound.setIntensity('normal');Sound.setPlaying(false);mountainState=null;gradientCache.clear();backgroundRenderer.invalidate();Object.assign(game,{mode:'menu',time:0,score:0,distance:0,speed:DIFF[difficulty].startSpeed,combo:1,comboT:0,energy:DIFF[difficulty].startEnergy,over:0,overReady:false,overWindow:0,overBreaks:0,overPulse:0,overFinish:0,signature:0,signaturePulse:0,tutorialGate:false,perfect:0,flow:1,flowPerfect:0,hitStop:0,spawn:.7,lastHazard:'',pendingPlan:null,generatorPhase:'intro',generatorCount:0,checkpointDistance:0,rescueAvailable:false,tutorialRun:false,tutorialStage:0,things:[],particles:[],notes:[],completed:false,newBest:false,challengeResult:null});Object.assign(p,{x:runnerX(),y:floor()-68,w:42,h:68,vy:0,jumps:0,ground:true,coyote:PHYSICS.coyoteTime,shield:0,magnet:0,slow:0,rot:0,land:0,takeoff:0,impact:0,hurt:0,iceDrift:0});runStats.finished=true;if(appUi)appUi.resetTransient();root.dataset.tutorialGate='off';root.dataset.signature='off';last=0;shake=0;flash=0}
  ...
  function setVisualQuality(next){next=Math.max(.58,Math.min(1,next));if(Math.abs(next-visualQuality)<.01)return;visualQuality=next;root.dataset.renderQuality=next>.9?'high':next>.68?'balanced':'low'}
  function resetAdaptiveQuality(){frameTimeEma=16.7;qualityPressure=0;qualityRecovery=0;setVisualQuality(performanceMode==='performance'?.62:1)}
  function updateAdaptiveQuality(rawDt){if(performanceMode!=='auto'){setVisualQuality(performanceMode==='performance'?.62:1);return}if(rawDt<=0||rawDt>.08)return;frameTimeEma+=(rawDt*1000-frameTimeEma)*.06;if(frameTimeEma>20.5){qualityPressure++;qualityRecovery=0}else if(frameTimeEma<17.8){qualityRecovery++;qualityPressure=Math.max(0,qualityPressure-2)}else{qualityPressure=Math.max(0,qualityPressure-1);qualityRecovery=Math.max(0,qualityRecovery-1)}if(qualityPressure>=45){setVisualQuality(visualQuality>.9?.78:.58);qualityPressure=0}else if(qualityRecovery>=240){setVisualQuality(visualQuality<.68?.78:1);qualityRecovery=0}}
  function updateHudTop(){const rootRect=root.getBoundingClientRect(),elements=[orientationButton,root.querySelector('.nr-brand'),root.querySelector('.nr-quick-controls')],bottom=elements.reduce((value,element)=>{if(!element)return value;const style=getComputedStyle(element);if(style.display==='none'||style.visibility==='hidden')return value;const rect=element.getBoundingClientRect();return Math.max(value,rect.bottom-rootRect.top)},0);cachedHudTop=Math.max(H<460||W<560?54:86,Math.ceil(bottom+10))}
  function resize(skipRender=false){const r=root.getBoundingClientRect(),nextW=Math.max(320,r.width),nextH=Math.max(280,r.height),nextDPR=Math.min(dprLimit(),Math.max(1,window.devicePixelRatio||1));updateOrientationUi();if(canvas.width&&Math.abs(nextW-sizedW)<1&&Math.abs(nextH-sizedH)<1&&Math.abs(nextDPR-DPR)<.01){updateHudTop();return}W=nextW;H=nextH;sizedW=W;sizedH=H;DPR=nextDPR;canvas.width=Math.round(W*DPR);canvas.height=Math.round(H*DPR);ctx.setTransform(DPR,0,0,DPR,0,0);ctx.imageSmoothingEnabled=true;if('imageSmoothingQuality' in ctx)ctx.imageSmoothingQuality=performanceMode==='performance'?'medium':'high';gradientCache.clear();backgroundRenderer.resize({width:W,height:H,dpr:DPR,theme:root.dataset.theme,quality:visualQuality});seed();p.x=runnerX();const map=challengeMap();if(map&&!mountainState)mountainState=map.createState(W,H);if(game.mode!=='play')p.y=floor()-p.h;updateHudTop();if(!skipRender)render()}
  ...
  function actionButton(y,color){ctx.save();ctx.shadowBlur=fxBlur(8);ctx.shadowColor=rgba(color,.55);const g=ctx.createLinearGradient(W/2-105,y,W/2+105,y+50);g.addColorStop(0,rgba(color,.78));g.addColorStop(1,rgba(C.card,.96));rr(W/2-105,y,210,50,14,g,rgba(color,.85));ctx.fillStyle=C.fg;ctx.globalAlpha=.72;ctx.beginPath();ctx.moveTo(W/2+78,y+18);ctx.lineTo(W/2+88,y+25);ctx.lineTo(W/2+78,y+32);ctx.strokeStyle=C.fg;ctx.lineWidth=2;ctx.stroke();ctx.restore()}
  ...
  function reset(){
    ...
    Object.assign(game,{mode:'play',time:0,score:0,distance:0,speed:cfg.startSpeed,combo:1,comboT:0,energy:cfg.startEnergy,over:0,overReady:false,overWindow:0,overBreaks:0,overPulse:0,overFinish:0,signature:0,signaturePulse:0,tutorialGate:false,perfect:0,flow:1,flowPerfect:0,hitStop:0,lastHazard:'',pendingPlan:null,generatorPhase:'intro',generatorCount:0,checkpointDistance:0,rescueAvailable:game.diff==='easy',tutorialRun:false,tutorialStage:0,things:[],particles:[],notes:[],completed:false,newBest:false,challengeResult:null});
    if(isMountain()){
    ...
    Object.assign(p,{x:runnerX(),y:floor()-68,w:42,h:68,vy:0,jumps:0,ground:true,coyote:PHYSICS.coyoteTime,jumpBuffer:0,jumpHeld:false,slide:0,slideState:'idle',dash:0,shield:game.diff==='easy'?2.5:0,magnet:0,slow:0,rot:0,land:0,takeoff:0,impact:0,hurt:0,iceDrift:0});for(let i=0;i<6;i++)coin(W+120+i*48,floor()-115-Math.sin(i*.7)*35);last=0;schedule();tone(520,.1,'square',.04);
    if(appUi){game.tutorialRun=appUi.startTutorial();setTutorialGate(game.tutorialRun)}
  }
  ...
  function showMenu(){stopLoop();clearInputState();modalResume=false;game.tutorialGate=false;root.dataset.tutorialGate='off';musicIntensity='normal';if(Sound.setIntensity)Sound.setIntensity('normal');game.mode='menu';game.completed=false;if(appUi){appUi.hideResults();appUi.closeSettings({resume:false,consumeHistory:false})}syncModeUi();render()}
  ...
  function performJump(){const second=p.jumps===1;p.vy=second?PHYSICS.secondJumpVelocity:PHYSICS.firstJumpVelocity;p.jumps=second?2:1;p.ground=false;p.coyote=0;p.jumpBuffer=0;p.slide=0;p.slideState='idle';p.takeoff=second?.16:.12;p.land=0;if(second&&!reducedMotion)p.rot=-.32;Sound.sfx(second?'doubleJump':'jump');if(appUi)appUi.tutorialSignal(second?'doubleJump':'jump');burst(p.x+20,p.y+p.h,second?C.two:C.one,second?13:8,second?.75:.55)}
  ...
  function pressJump(){if(game.mode==='menu'||game.mode==='over'){reset();return}if(game.mode!=='play')return;if(game.tutorialGate)releaseTutorialGate();if(isMountain()){mountainInput.jumpHeld=true;mountainInput.jumpPressed=true;return}p.jumpHeld=true;p.jumpBuffer=PHYSICS.jumpBuffer;tryBufferedJump()}
  function releaseJump(){if(isMountain()){mountainInput.jumpHeld=false;return}p.jumpHeld=false;if(p.vy<-220)p.vy*=.5}
  function slide(on=true){if(game.mode!=='play')return;if(isMountain()){mountainInput.down=on;return}if(on){if(p.ground&&p.slide<=0){p.slide=PHYSICS.slideDuration;p.slideState='start';p.impact=.09;Sound.sfx('slide');if(appUi)appUi.tutorialSignal('slide');burst(p.x+8,p.y+p.h,C.three,7,.38)}}else if(p.slide>0){p.slide=Math.min(p.slide,.12);p.slideState='end'}}
  function activateOverdrive(){if(!game.overReady||game.over>0)return false;game.overReady=false;game.overWindow=0;game.over=OVERDRIVE_DURATION;game.overBreaks=0;game.overPulse=.42;game.energy=0;root.dataset.overdrive='on';refreshMusicIntensity();if(appUi)appUi.tutorialSignal('overdrive');note(tr('overdriveOn'),C.three);Sound.sfx('overdrive');shake=reducedMotion?0:scene==='volcano'?2:3;burst(p.x+p.w/2,p.y+p.h/2,C.three,22,.9);return true}
  function dash(){if(game.mode!=='play')return;if(isMountain()){mountainInput.dashPressed=true;return}if(activateOverdrive())return;if(p.dash>0||game.energy<PHYSICS.dashCost)return;game.energy-=PHYSICS.dashCost;p.dash=PHYSICS.dashDuration;p.impact=.12;p.takeoff=0;shake=reducedMotion?0:scene==='volcano'?2:3.5;Sound.sfx('dash');vibrate(12);burst(p.x,p.y+p.h/2,C.three,18,.85)}
  ...
  if(globalThis.PointerEvent){
    ...
    root.querySelectorAll('[data-act]').forEach(button=>{const action=button.dataset.act;button.addEventListener('pointerdown',event=>{event.preventDefault();event.stopPropagation();if(activeControlPointers.has(event.pointerId))return;activeControlPointers.set(event.pointerId,{button,action});button.classList.add('is-pressed');try{button.setPointerCapture(event.pointerId)}catch(error){}actionStart(action)});for(const eventName of ['pointerup','pointercancel','lostpointercapture'])button.addEventListener(eventName,event=>{const active=activeControlPointers.get(event.pointerId);if(!active||active.button!==button)return;event.preventDefault();event.stopPropagation();activeControlPointers.delete(event.pointerId);button.classList.remove('is-pressed');actionEnd(active.action)})});
    root.querySelectorAll('[data-act],[data-quick],[data-orientation-toggle]').forEach(button=>button.addEventListener('contextmenu',event=>event.preventDefault()));
  }else{
  ...
  function hit(o){
    ...
    if((p.dash>0||game.over>0)&&profile.dashBreakable){
      ...
      game.hitStop=reducedMotion?.018:game.over>0?.052:.042;p.impact=.14;shake=reducedMotion?0:scene==='volcano'?2:5;burst(o.x+o.w/2,o.y+o.h/2,profile.electronic?C.three:C.two,22,.95);Sound.sfx('break');
      if(game.over>0){game.overBreaks++;game.overPulse=.3;tone(260+Math.min(7,game.overBreaks)*48,.055,'triangle',.025+.002*game.overBreaks)}
  ...
  function update(dt){
    if(game.tutorialGate)return;
    p.impact=Math.max(0,(p.impact||0)-dt);p.takeoff=Math.max(0,(p.takeoff||0)-dt);if(game.hitStop>0){game.hitStop=Math.max(0,game.hitStop-dt);return}
    ...
    if(game.energy>=100&&game.over<=0&&!game.overReady){game.overReady=true;game.overWindow=OVERDRIVE_READY_WINDOW;root.dataset.overdrive='ready';refreshMusicIntensity();note(tr('novaReady'),C.three);Sound.sfx('powerup');vibrate(10)}
    if(game.overReady){game.overWindow=Math.max(0,game.overWindow-dt);if(game.overWindow<=0)activateOverdrive()}
    const wasOver=game.over>0;game.over=Math.max(0,game.over-dt);if(wasOver&&game.over<=0){root.dataset.overdrive='off';game.overFinish=.62;refreshMusicIntensity();if(game.overBreaks>0){note(`${tr('novaFlow')} ×${game.overBreaks}`,C.three);tone(260+Math.min(6,game.overBreaks)*55,.13,'triangle',.04);burst(p.x+p.w/2,p.y+p.h/2,C.three,16,.65)}}p.dash=Math.max(0,p.dash-dt);p.slide=Math.max(0,p.slide-dt);p.slideState=p.slide<=0?'idle':p.slide>.38?'start':p.slide>.12?'active':'end';p.jumpBuffer=Math.max(0,p.jumpBuffer-dt);p.shield=Math.max(0,p.shield-dt);p.magnet=Math.max(0,p.magnet-dt);p.slow=Math.max(0,p.slow-dt);game.overPulse=Math.max(0,game.overPulse-dt);game.overFinish=Math.max(0,game.overFinish-dt);game.signaturePulse=Math.max(0,game.signaturePulse-dt);const hadSignature=game.signature>0;game.signature=Math.max(0,game.signature-dt);if(hadSignature&&game.signature<=0){root.dataset.signature='off';refreshMusicIntensity()}game.flowPerfect=Math.max(0,game.flowPerfect-dt);game.comboT-=dt;if(game.comboT<=0)game.combo=Math.max(1,game.combo-dt*4);game.spawn-=dt*boost;if(game.spawn<=0)spawn();
    ...
    if(!gap&&p.y+p.h>=floor()&&p.vy>=0){
      ...
      if(!wasGround&&landingSpeed>280){p.land=Math.min(.18,.085+landingSpeed/9000);p.takeoff=0;if(scene==='snow')p.iceDrift=Math.min(18,Math.max(p.iceDrift,landingSpeed*.014));burst(p.x+p.w/2,floor(),C.one,6,.35);tone(120,.045,'sine',.018)}
  ...
  function drawStateAtmosphere(){if(game.mode==='play')StateEffects.draw(ctx,{player:p,game,scene,width:W,height:H,palette:C,reducedMotion,performanceMode,visualQuality})}
  ...
  function drawThings(){
    for(const o of game.things){
      ...
      if(o.type==='coin'){
        ctx.translate(o.x+11,o.y+11);ctx.scale(.25+Math.abs(Math.cos(o.spin))*.75,1);ctx.shadowBlur=fxBlur(18);ctx.shadowColor=C.three;ctx.fillStyle=C.three;ctx.beginPath();ctx.arc(0,0,10,0,7);ctx.fill();ctx.fillStyle=C.fg;ctx.globalAlpha=.55;ctx.beginPath();ctx.arc(-2,-2,4,0,7);ctx.fill();
      }else if(o.type==='crate'){
        ctx.shadowBlur=fxBlur(18);ctx.shadowColor=C.two;rr(o.x,o.y,o.w,o.h,8,rgba(C.two,.2),C.two);ctx.strokeStyle=rgba(C.two,.6);ctx.beginPath();ctx.moveTo(o.x+8,o.y+8);ctx.lineTo(o.x+o.w-8,o.y+o.h-8);ctx.moveTo(o.x+o.w-8,o.y+8);ctx.lineTo(o.x+8,o.y+o.h-8);ctx.stroke();
      }else if(o.type==='spike'){
        ctx.shadowBlur=fxBlur(16);ctx.shadowColor=C.two;ctx.fillStyle=C.two;for(let i=0;i<3;i++){ctx.beginPath();ctx.moveTo(o.x+i*22,o.y+o.h);ctx.lineTo(o.x+i*22+11,o.y);ctx.lineTo(o.x+i*22+22,o.y+o.h);ctx.fill()}
      }else if(o.type==='drone'){
        ctx.shadowBlur=fxBlur(18);ctx.shadowColor=C.one;rr(o.x,o.y,o.w,o.h,14,rgba(C.one,.18),C.one);ctx.fillStyle=C.two;ctx.fillRect(o.x+18,o.y+12,22,7);ctx.strokeStyle=C.one;ctx.beginPath();ctx.moveTo(o.x-16,o.y+10);ctx.lineTo(o.x+4,o.y+10);ctx.moveTo(o.x+o.w-4,o.y+10);ctx.lineTo(o.x+o.w+16,o.y+10);ctx.stroke();
      }else if(HAZARDS[o.type]&&HAZARDS[o.type].gapLike){
        drawGapHazard(o);
      }else if(o.type==='spring'||o.type==='waterPlatform'){
        const color=o.type==='waterPlatform'?C.one:C.three;ctx.shadowBlur=fxBlur(16);ctx.shadowColor=color;rr(o.x,o.y,o.w,o.h,5,rgba(color,.25),color);ctx.fillStyle=color;for(let i=0;i<3;i++)ctx.fillRect(o.x+8+i*(o.w-16)/3,o.y-4,8,7);
      }else if(o.type==='gate'||o.type==='shutter'){
        const color=o.type==='shutter'?C.one:C.two;ctx.shadowBlur=fxBlur(18);ctx.shadowColor=color;ctx.fillStyle=color;ctx.fillRect(o.x,0,o.w,o.gapY);ctx.fillRect(o.x,o.gapY+o.gapH,o.w,floor()-o.gapY-o.gapH);ctx.fillStyle=C.fg;ctx.globalAlpha=.72;for(let y=8;y<floor();y+=18)ctx.fillRect(o.x+4,y,o.w-8,2);ctx.globalAlpha=1;ctx.shadowBlur=fxBlur(10);ctx.shadowColor=C.three;ctx.strokeStyle=C.three;ctx.lineWidth=4;ctx.strokeRect(o.x-5,o.gapY-4,o.w+10,o.gapH+8);ctx.fillStyle=C.three;ctx.beginPath();ctx.moveTo(o.x-18,o.gapY+o.gapH/2);ctx.lineTo(o.x-30,o.gapY+o.gapH/2-9);ctx.lineTo(o.x-30,o.gapY+o.gapH/2+9);ctx.closePath();ctx.fill();
      }else if(o.type==='billboard'){
        ctx.shadowBlur=fxBlur(16);ctx.shadowColor=C.one;rr(o.x,o.y,o.w,o.h,6,rgba(C.one,.2),C.one);ctx.fillStyle=C.two;ctx.fillRect(o.x+8,o.y+10,o.w-16,8);ctx.fillStyle=C.fg;ctx.globalAlpha=.7;ctx.fillRect(o.x+8,o.y+27,o.w*.55,4);ctx.strokeStyle=C.one;ctx.beginPath();ctx.moveTo(o.x+14,o.y+o.h);ctx.lineTo(o.x+14,floor());ctx.moveTo(o.x+o.w-14,o.y+o.h);ctx.lineTo(o.x+o.w-14,floor());ctx.stroke();
      }else if(o.type==='bridgeBar'){
        ctx.shadowBlur=fxBlur(14);ctx.shadowColor=C.two;rr(o.x,o.y,o.w,o.h,4,rgba(C.card,.96),C.two);ctx.fillStyle=C.two;for(let x=o.x+5;x<o.x+o.w-8;x+=18)fillPath([[x,o.y+3],[x+8,o.y+o.h-3],[x+15,o.y+3]],C.two);
      }else if(o.type==='train'){
        ctx.shadowBlur=fxBlur(18);ctx.shadowColor=C.two;rr(o.x,o.y,o.w,o.h,12,rgba(C.card,.96),C.two);ctx.fillStyle=C.one;for(let x=o.x+14;x<o.x+o.w-18;x+=25)rr(x,o.y+10,16,13,3,rgba(C.one,.7));ctx.fillStyle=C.two;ctx.fillRect(o.x+8,o.y+o.h-10,o.w-16,4);ctx.strokeStyle=C.fg;ctx.beginPath();ctx.moveTo(o.x+o.w-12,o.y+8);ctx.lineTo(o.x+o.w,o.y+o.h/2);ctx.lineTo(o.x+o.w-12,o.y+o.h-6);ctx.stroke();
      }else if(o.type==='laser'){
        ctx.shadowBlur=fxBlur(20);ctx.shadowColor=C.three;rr(o.x,o.y,10,o.h,4,rgba(C.card,.95),C.three);rr(o.x+o.w-10,o.y,10,o.h,4,rgba(C.card,.95),C.three);ctx.strokeStyle=C.two;ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(o.x+10,o.y+o.h*.5);ctx.lineTo(o.x+o.w-10,o.y+o.h*.5);ctx.stroke();
      }else if(o.type==='robot'){
        ctx.shadowBlur=fxBlur(16);ctx.shadowColor=C.three;rr(o.x+6,o.y+12,o.w-12,o.h-12,9,rgba(C.card,.94),C.three);rr(o.x,o.y,o.w,25,8,rgba(C.one,.25),C.one);ctx.fillStyle=C.two;ctx.fillRect(o.x+11,o.y+9,7,5);ctx.fillRect(o.x+o.w-18,o.y+9,7,5);ctx.strokeStyle=C.three;ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(o.x+13,o.y+o.h-3);ctx.lineTo(o.x+7,o.y+o.h+5);ctx.moveTo(o.x+o.w-13,o.y+o.h-3);ctx.lineTo(o.x+o.w-7,o.y+o.h+5);ctx.stroke();
      }else if(o.type==='scanner'){
        ctx.shadowBlur=fxBlur(18);ctx.shadowColor=C.three;rr(o.x,o.y,o.w,o.h,8,rgba(C.card,.94),C.three);ctx.fillStyle=C.one;ctx.fillRect(o.x+12,o.y+10,o.w-24,5);ctx.fillStyle=rgba(C.three,.12);ctx.fillRect(o.x+8,o.y+o.h,o.w-16,floor()-o.y-o.h);ctx.strokeStyle=rgba(C.three,.55);ctx.beginPath();ctx.moveTo(o.x+o.w/2,o.y+o.h);ctx.lineTo(o.x+o.w/2,floor());ctx.stroke();
      }else if(o.type==='snowball'||o.type==='rock'){
        const color=o.type==='snowball'?C.two:C.two;ctx.shadowBlur=fxBlur(14);ctx.shadowColor=color;ctx.fillStyle=o.type==='snowball'?rgba(C.fg,.94):rgba(C.card,.94);ctx.strokeStyle=color;ctx.lineWidth=3;ctx.beginPath();ctx.arc(o.x+o.w/2,o.y+o.h/2,o.w*.46,0,7);ctx.fill();ctx.stroke();ctx.beginPath();ctx.arc(o.x+o.w/2,o.y+o.h/2,o.w*.23,(o.spin||game.time)*2,(o.spin||game.time)*2+4);ctx.stroke();
      }else if(o.type==='icicle'){
        ctx.shadowBlur=fxBlur(16);ctx.shadowColor=C.two;fillPath([[o.x,o.y],[o.x+o.w,o.y],[o.x+o.w*.54,o.y+o.h]],rgba(C.fg,.9),C.two);ctx.fillStyle=C.two;ctx.fillRect(o.x,o.y,o.w,4);
      }else if(o.type==='flame'){
        ctx.shadowBlur=fxBlur(18);ctx.shadowColor=C.two;fillPath([[o.x+o.w*.5,o.y],[o.x+o.w*.78,o.y+24],[o.x+o.w,o.y+o.h],[o.x,o.y+o.h],[o.x+o.w*.2,o.y+31]],rgba(C.two,.8),C.three);ctx.fillStyle=rgba(C.fg,.6);ctx.beginPath();ctx.ellipse(o.x+o.w*.5,o.y+o.h*.68,o.w*.16,o.h*.24,0,0,7);ctx.fill();
      }else if(o.type==='log'){
        ...
        ctx.shadowBlur=fxBlur(12);ctx.shadowColor=C.one;rr(o.x,o.y,o.w,o.h,8,rgba(C.one,.35),C.fg);ctx.fillStyle=rgba(C.one,.18);ctx.fillRect(o.x+8,o.y+o.h,o.w-16,floor()-o.y-o.h);ctx.strokeStyle=rgba(C.fg,.7);for(let x=o.x+12;x<o.x+o.w-8;x+=15){ctx.beginPath();ctx.moveTo(x,o.y+4);ctx.lineTo(x-5,o.y+o.h-4);ctx.stroke()}
      }else{
        const col=o.type==='shield'?C.one:o.type==='magnet'?C.two:C.four;ctx.shadowBlur=fxBlur(22);ctx.shadowColor=col;ctx.fillStyle=rgba(C.card,.85);ctx.strokeStyle=col;ctx.lineWidth=3;ctx.beginPath();ctx.arc(o.x+17,o.y+17,17,0,7);ctx.fill();ctx.stroke();ctx.fillStyle=C.fg;ctx.font='500 17px system-ui';ctx.textAlign='center';ctx.fillText(o.type==='shield'?'◆':o.type==='magnet'?'U':'◷',o.x+17,o.y+23);
      }
      ctx.restore();
    }
  }
  ...
  function hudTop(){return cachedHudTop}
  ...
  function hud(){
    ...
    for(const n of game.notes){ctx.globalAlpha=Math.min(1,n.t*2);ctx.textAlign='center';fitFont(n.text,W*.74,compact?21:27,12,'700');ctx.fillStyle=n.color;ctx.shadowBlur=fxBlur(8);ctx.shadowColor=n.color;ctx.fillText(n.text,W/2,n.y)}ctx.restore()
  }
  ...
  function render(){ctx.clearRect(0,0,W,H);ctx.save();C.skin=skinAccent();if(isMountain()){const map=challengeMap();mountainState=mountainState||map.createState(W,H);mountainState.reducedMotion=reducedMotion;mountainState.renderQuality=visualQuality;mountainState.performanceMode=performanceMode;map.render(ctx,mountainState,W,H,C);ctx.save();ctx.translate(-mountainState.cameraX,0);PlayerRenderer.drawChallenge(ctx,mountainState,C,skinProfile());ctx.restore();overlay();if(flash&&!reducedMotion){ctx.globalAlpha=flash;ctx.fillStyle=C.fg;ctx.fillRect(0,0,W,H)}ctx.restore();return}if(shake&&!reducedMotion&&shakeStrength>0)ctx.translate((Math.random()-.5)*shake*shakeStrength,(Math.random()-.5)*shake*shakeStrength);background();drawSpeedLines();drawThings();drawEffects();player();drawStateAtmosphere();hud();overlay();if(flash&&!reducedMotion){ctx.globalAlpha=flash;ctx.fillStyle=C.fg;ctx.fillRect(0,0,W,H)}ctx.restore()}
  function stopLoop(){if(frameId){caf(frameId);frameId=0}scheduled=false;last=0}
  function schedule(){if(!scheduled&&visible&&!document.hidden&&game.mode==='play'&&!game.tutorialGate){scheduled=true;frameId=raf(loop)}}
  function loop(t){scheduled=false;frameId=0;if(game.mode!=='play'||game.tutorialGate||document.hidden||!visible)return;const rawDt=last?(t-last)/1000:0,dt=Math.min(.033,rawDt);last=t;updateAdaptiveQuality(rawDt);try{update(dt);render()}catch(error){console.error('NOVA RUN frame error',error);visible=false;const panel=root.querySelector('[data-error]');if(panel){panel.hidden=false;panel.textContent='运行出现问题，请刷新页面后重试。';panel.dataset.detail=String(error&&error.message||error)}return}schedule()}
  document.addEventListener('visibilitychange',()=>{if(document.hidden){stopLoop();clearInputState();Sound.setPlaying(false)}else{last=0;Sound.setPlaying(game.mode==='play'&&!game.tutorialGate);schedule()}});
  ...
  appUi=UI.create({root,storage:Store,audio:Sound,tr,actions:{setting:applySetting,previewSetting,sceneSelect:chooseScene,retry:reset,menu:showMenu,scene:()=>{showMenu();appUi.openSettings()},pause,modalOpen,modalClose,tutorialSkip:releaseTutorialGate,enterFullscreen:()=>platform?.enterFullscreen(),exitFullscreen:()=>platform?.exitFullscreen(),installApp:()=>platform?.promptInstall(),applyUpdate:()=>platform?.applyUpdate()}});
