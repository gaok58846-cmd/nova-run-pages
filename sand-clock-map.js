...
function render(ctx,state,width,height,theme,api){
  const {floorFor,platformRect,polygon,rounded,drawFinishGate}=api,floor=floorFor(height),camera=state.cameraX,skyKey=`${width}|${height}`;if(state._skyKey!==skyKey){state._skyKey=skyKey;state._skyGradient=ctx.createLinearGradient(0,0,0,height);state._skyGradient.addColorStop(0,'#ead5ad');state._skyGradient.addColorStop(.58,'#d6a86e');state._skyGradient.addColorStop(1,'#5d4937')}ctx.fillStyle=state._skyGradient;ctx.fillRect(0,0,width,height);ctx.fillStyle='rgba(255,237,190,.8)';ctx.beginPath();ctx.arc(width*.22,height*.2,Math.min(width,height)*.105,0,Math.PI*2);ctx.fill();
