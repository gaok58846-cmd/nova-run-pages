...
const activeBoosting=
  ...
  ctx.save();ctx.translate(p.x+p.w/2,py+h/2);if(!reducedMotion&&landing>0){const pulse=Math.sin(landing*Math.PI);ctx.translate(0,pulse*3);ctx.scale(1+pulse*.075,1-pulse*.075)}if(!reducedMotion&&takeoff>0){const pulse=Math.sin(takeoff*Math.PI);ctx.scale(1-pulse*.035,1+pulse*.06)}if(!reducedMotion&&impact>0){const pulse=Math.sin(impact*Math.PI);ctx.translate(pulse*2.5,0);ctx.scale(1+pulse*.045,1-pulse*.025)}if(p.hurt)ctx.rotate(-.48);else if(p.dash>0)ctx.rotate(.09);else if(sliding&&!reducedMotion)ctx.rotate(p.slideState==='start'?-0.035:0);else if(!p.ground&&!reducedMotion)ctx.rotate(secondJump?Math.sin(game.time*10)*.16:p.vy/5200);else if(p.ground&&!sliding&&moving)ctx.rotate(motion.lean*(reducedMotion?.45:1));if(p.ground&&!sliding&&!p.hurt&&moving)ctx.translate(0,Math.abs(runPhase)*motion.bob);ctx.translate(0,-h*(visualScale-1)/2);ctx.scale(visualScale,visualScale);ctx.translate(-p.w/2,-h/2);
...
ctx.shadowColor =
  ...
  if(sliding){const reach=39+motion.intensity*8;ctx.strokeStyle=activeAccent;ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(8,29);ctx.lineTo(29,29);ctx.lineTo(reach,34);ctx.moveTo(14,24);ctx.lineTo(4-motion.intensity*4,32);ctx.stroke();rounded(ctx,10,12,27,17,8,S.body,activeAccent);rounded(ctx,18,3,19,16,7,S.shade,activeAccent);rounded(ctx,23,7,13,5,2,S.visor);ctx.fillStyle=activeAccent;ctx.fillRect(8,27,28,3)}else{
    ...
    ctx.save();ctx.shadowBlur=0;ctx.strokeStyle=S.shade;ctx.lineWidth=10;ctx.beginPath();ctx.moveTo(16,40);ctx.lineTo(13+legA*.35,55);ctx.lineTo(8+legA,66);ctx.moveTo(27,40);ctx.lineTo(29+legB*.35,55);ctx.lineTo(34+legB,66);ctx.stroke();ctx.lineWidth=9;ctx.beginPath();ctx.moveTo(13,25);ctx.lineTo(7+armA*.35,36-liftA*.45+dropA*.4);ctx.lineTo(6+armA,45-liftA+dropA);ctx.moveTo(30,25);ctx.lineTo(35+armB*.35,35-liftB*.45+dropB*.4);ctx.lineTo(36+armB,43-liftB+dropB);ctx.stroke();ctx.restore();
  ...
  fatal
    ...
    :game.over>0
      ?C.three
      :S.core;if(S.pattern==='pulse'){ctx.beginPath();ctx.arc(22,30,6,0,7);ctx.fill();ctx.strokeStyle=S.particle;ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(22,30,9,0,7);ctx.stroke()}else if(S.pattern==='sunset'){ctx.beginPath();for(let i=0;i<6;i++){const a=-Math.PI/2+i*Math.PI/3,x=22+Math.cos(a)*7,y=30+Math.sin(a)*7;i?ctx.lineTo(x,y):ctx.moveTo(x,y)}ctx.closePath();ctx.fill()}else if(S.pattern==='prism'){ctx.beginPath();ctx.moveTo(22,23);ctx.lineTo(29,35);ctx.lineTo(15,35);ctx.closePath();ctx.fill()}else{ctx.beginPath();ctx.moveTo(22,24);ctx.lineTo(28,30);ctx.lineTo(22,36);ctx.lineTo(16,30);ctx.closePath();ctx.fill()}ctx.strokeStyle=S.body;ctx.lineWidth=1;ctx.stroke();ctx.fillStyle=S.shade;ctx.strokeStyle=activeAccent;ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(13,12);ctx.quadraticCurveTo(12,2,22,0);ctx.quadraticCurveTo(33,2,32,12);ctx.quadraticCurveTo(29,20,21,20);ctx.quadraticCurveTo(14,18,13,12);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle=S.shade;ctx.beginPath();ctx.moveTo(13,5);ctx.lineTo(8,9);ctx.lineTo(13,14);ctx.closePath();ctx.fill();rounded(ctx,16,6,17,7,3,S.visor);ctx.fillStyle=S.body;ctx.globalAlpha=.8;ctx.fillRect(27,8,4,2);ctx.globalAlpha=1;ctx.shadowBlur=0;rounded(ctx,7,21,7,13,3,S.shade,activeAccent);rounded(ctx,30,20,7,13,3,S.shade,activeAccent);rounded(ctx,4+legA,62,11,6,3,S.shade,activeAccent);rounded(ctx,30+legB,62,11,6,3,S.shade,activeAccent);
  }
  if(fatal){
  drawCoreShutdown(
    ctx,
