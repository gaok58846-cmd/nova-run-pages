...
function render(ctx,state,width,height,theme,api){
  const {floorFor,platformRect,polygon,rounded,drawFinishGate}=api,floor=floorFor(height),level=waterLevel(state,height),camera=state.cameraX,skyKey=`${width}|${height}`;if(state._skyKey!==skyKey){state._skyKey=skyKey;state._skyGradient=ctx.createLinearGradient(0,0,0,height);state._skyGradient.addColorStop(0,'#efe5cf');state._skyGradient.addColorStop(.62,'#cddfd6');state._skyGradient.addColorStop(1,'#507b7c')}ctx.fillStyle=state._skyGradient;ctx.fillRect(0,0,width,height);
